...
public class CommandSignsSignClickEvent {
    ...
    public void onRightClick(PlayerInteractEvent event, Block sign) {
        ...
        if (state != null) {
            ...
            } else if (state.equals(CommandSignsPlayerState.EDIT)) {
                editSign(player, location);
            }
            return;
        }
    ...
    public boolean editSign(Player player, Location loc) {
        ...
        String[] lines = HashMaps.getSignText(loc).getText();
        player.sendMessage("You may now edit this sign: ");
        for (int i = 0; i < lines.length; i++) {
            if (lines[i] != null) {
                player.sendMessage("Line" + i + ": " + lines[i]);
            }
